...
function BrandDashboardContent() {
  ...
  return (
    <div className="h-screen bg-stone-100 text-stone-900 flex flex-col font-rethink">
      ...
      <div className="fixed bottom-6 left-6 z-50 bg-stone-900 text-white px-4 py-2.5 rounded-full flex items-center gap-3 text-xs font-semibold border border-stone-800">
        <span>State: {dashboardState === "empty" ? "Empty State" : "Active Dashboard"}</span>
        <button
          onClick={() => setDashboardState(prev => (prev === "empty" ? "active" : "empty"))}
          className="bg-[#FEB604] text-stone-900 px-3 py-1 rounded-full"
        >
          Toggle Screen
        </button>
      </div>
      ...
      {activeTab === "home" ? (
        ...
        <main className="flex-1 flex flex-col items-center justify-center max-w-7xl w-full mx-auto px-6 py-12">
          <div className="text-center max-w-md bg-white border border-stone-200 rounded-2xl p-8">
            ...
            <span className="text-xs font-medium px-2.5 py-1 bg-stone-100 rounded-full text-stone-600">Balance Locked</span>
          </div>
        </main>
      )}
    </div>
...
export default function BrandDashboard() {
  return (
      <Suspense fallback={<div className="min-h-screen bg-stone-100 flex items-center justify-center font-rethink text-stone-500">Loading Dashboard...</div>}>
