...
export default function CreateAccountPage() {
  ...
  const handleRoleContinue = () => {
    if (role === "creator") {
      ...
      alert("Business flows are on the brand dashboard. Redirecting...");
      window.location.href = `${window.location.protocol}//${window.location.hostname}:3002/create-account`;
    }
  };
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      await apiRequest("/auth/register", {
        method: "POST",
        body: JSON.stringify({
          name: form.nickname || fullName,
          displayName: form.nickname || fullName,
